...
function storeReducer(action, state) {
	const type = action.type;
		switch(type) {
				case 'ADD_TODOS': {
					return Object.assign({}, {...state, todo: [...state.todo.items, action.payload]});
				}
				case 'INCREMENT': {
...
const unsub = store.subscribe((actionType) => {
	console.log('Mutate', {type: actionType.type, payload: actionType.payload})
});

store.dispatch(actions.addItems({text: 'aaaa'}));
